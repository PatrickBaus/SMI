...
function getUnits($con) {
  ...
  return $con->query($query_get_units)->fetchAll(PDO::FETCH_KEY_PAIR);
}
...
function getNodes($con) {
  ...
  return $con->query($query_get_nodes)->fetchAll(PDO::FETCH_KEY_PAIR);
}

// Extract all available rooms
function getRooms($con) {
  global $query_get_rooms;
  return $con->query($query_get_rooms)->fetchAll(PDO::FETCH_KEY_PAIR);
}
